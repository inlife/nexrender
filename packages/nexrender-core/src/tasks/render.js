...
module.exports = (job, settings) => {
    ...
    const parse = data => {
        ...
        const matchStart = isNaN(parseInt(projectStart))
            ? startRegex.exec(string)
            : false
        // Only execute durationRegex if project duration hasnt been found
        const matchDuration = isNaN(parseInt(projectDuration))
            ? durationRegex.exec(string)
            : false
        // Only execute progressRegex if project duration has been found
        const matchProgress = !isNaN(parseInt(projectDuration))
            ? progressRegex.exec(string)
            : null
        // If durationRegex has a match convert tstamp to seconds and define projectDuration only once
        projectDuration = matchDuration
            ? seconds(matchDuration[1])
            : projectDuration
        ...
        if (matchProgress) {
            currentProgress = Math.ceil(
                ((seconds(matchProgress[1]) - projectStart) * 100) /
                    projectDuration
            )

            if (previousProgress !== currentProgress) {
                settings.logger.log(
                    `[${job.uid}] rendering progress ${currentProgress}%...`
                )
    ...
    return new Promise((resolve, reject) => {
        ...
        instance.on('close', code => {
            ...
            settings.logger.log(
                `[${job.uid}] rendering took ~${
                    (Date.now() - renderStopwatch) / 1000
                } sec.`
            )
            settings.logger.log(
                `[${job.uid}] writing aerender job log to: ${logPath}`
            )
            ...
            if (!fs.existsSync(outputFile)) {
                ...
                return reject(
                    new Error(`Couldn't find a result file: ${outputFile}`)
                )
            }
            ...
            if (stats.size < 1000) {
                settings.logger.log(
                    `[${job.uid}] Warning: output file size is less than 1000 bytes (${stats.size} bytes), be advised that file is corrupted, or rendering is still being finished`
                )
            }
